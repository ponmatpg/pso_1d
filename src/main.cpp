...
public:
  Particle(int _index, double _position, double _velocity, double _c1, double _c2, double (*_f)(double))
      : index(_index), position(_position), velocity(_velocity), c1(_c1), c2(_c2), f(_f) {
    value = f(position);
    _best_pos = position;
  ...
  void step(
    ...
    velocity = step_weight * velocity + c1 * r1 * (_best_pos - position) + c2 * r2 * (best_global_pos - position);
...
public:
  PSO(double initial_pos, double _c1, double _c2, double (*_f)(double), int _n_particles): c1(_c1), c2(_c2), f(_f), n_particles(_n_particles) {
    steps = 0;
  ...
  void print_global(){
    std::cout << 
    "Particle Max Dist: " << particle_max_dist() << std::endl << 
    "Particle Value Max Dist: " << particle_value_max_dist() << std::endl << 
  ...
  void run(double threshold, int max_steps){
    while (steps < max_steps){
      step(steps);
      ++steps;
      if ((particle_value_max_dist() < threshold) && (particle_max_dist() < threshold))
        break;
    }
  }
